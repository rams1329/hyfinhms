...
const Dashboard = () => {
  ...
  // Filter doctorStats based on selectedDoctor
  const filteredDoctorStats = selectedDoctor === 'all' ? doctorStats : doctorStats.filter(doc => doc.id === selectedDoctor);

  // Doctor comparison chart data
  const doctorComparisonData = {
    labels: filteredDoctorStats.map(doc => doc.name),
    datasets: [
      {
        label: 'Total Appointments',
        data: filteredDoctorStats.map(doc => doc.total),
      ...
      {
        label: 'Completed',
        data: filteredDoctorStats.map(doc => doc.completed),
        backgroundColor: 'rgba(75, 192, 192, 0.6)',
        borderColor: 'rgba(75, 192, 192, 1)',
        borderWidth: 1,
      },
      {
        label: 'Cancelled',
        data: filteredDoctorStats.map(doc => doc.cancelled),
      ...
      <div className="m-5">
        ...
        <div className="bg-white p-4 rounded-lg shadow-sm border border-gray-100 mb-8">
          <div className="flex items-center justify-between mb-4">
            <h3 className="text-lg font-semibold text-gray-700">Doctor Performance</h3>
            <select
